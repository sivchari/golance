...
func TestPhaseTimer_DominantPhase(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if dur != 10*time.Millisecond {
			t.Errorf("dominant() duration = %s, want %s", dur, 10*time.Millisecond)
		}
	})
}
...
func TestServerInstrument_LogsSlowRequestWithDominantPhase(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if !strings.Contains(logged, "duration=510ms") {
			t.Errorf("log line %q does not report the exact duration", logged)
		}
		if !strings.Contains(logged, "dominant_phase=depcheck.check") {
			t.Errorf("log line %q does not mention the dominant phase", logged)
		}
		if !strings.Contains(logged, "dominant_phase_duration=510ms") {
			t.Errorf("log line %q does not report the exact dominant phase duration", logged)
		}
	})
}
